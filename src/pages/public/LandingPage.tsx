        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {records.map((record, index) => {
              ...
              const displayableImageTypes = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp', 'svg'];
              const firstImageAsset = record.fileAssets?.find((file) => {
                const isImage = file.mimeType?.startsWith('image/');
                const fileExtension = file.mimeType?.split('/')[1]?.toLowerCase();
                return isImage && displayableImageTypes.includes(fileExtension || '');
              });
